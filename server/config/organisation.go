...
func CheckSuperOrganisation() bool {
	...
	req, _ := http.NewRequest("GET", viper.GetString("keto_url")+ketoPolicyPath+"/app:vidcheck:superorg", nil)
...
func createSuperOrganisationPermissions(oID uint) error {
	return model.DB.Model(&OrganisationPermission{}).Create(&OrganisationPermission{
		OrganisationID: oID,
		Spaces:         -1,
	}).Error
}

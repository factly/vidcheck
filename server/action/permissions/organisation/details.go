...
func details(w http.ResponseWriter, r *http.Request) {
	...
	resp, _ := util.KetoGetRequest("/engines/acp/ory/regex/policies/app:vidcheck:superorg")
	...
	err = model.DB.Model(&model.SpacePermission{}).Where("space_id IN (?)", spaceIDs).Find(&result.SpacePermissions).Error
	if err != nil {
		loggerx.Error(err)

...
func update(w http.ResponseWriter, r *http.Request) {
	...
	if category.ParentID != 0 {
		...
		err = model.DB.Where(&model.Category{SpaceID: uint(sID)}).First(&parentCat).Error
	...
	if result.Slug == category.Slug {
		...
		categorySlug = slugx.Approve(&model.DB, category.Slug, sID, tableName)
	} else {
		categorySlug = slugx.Approve(&model.DB, slugx.Make(category.Name), sID, tableName)
	}
	...
	if len(category.Description.RawMessage) > 0 && !reflect.DeepEqual(category.Description, util.NilJsonb()) {

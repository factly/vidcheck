...
export default class Embed {
  ...
  makeCaptionHolder() {
    ...
    this.nodes.caption = this.make(
      "div",
      [this.CSS.input, this.CSS.captionEl],
      {
        contentEditable: !this.readOnly,
  ...
  onFetch(response) {
    ...
    if (!metaData) {
      this.fetchingFailed(
        this.api.i18n.t("Couldn't get meta data from the server")
      );
    ...
    if (!html) {
      this.fetchingFailed(
        this.api.i18n.t("Couldn't get html data from the server")
      );
  ...
  save() {
    const caption = this.nodes.captionHolder.querySelector(
      `.${this.CSS.captionEl}`
    );
